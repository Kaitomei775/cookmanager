...
public class Make_DB {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		/*
		for (int k=0; i<recipes.size(); ++i){ //動作チェック
		    System.out.println(recipes.get(i).recipe_url);
		    System.out.println(recipes.get(i).recipe_image_url);
		    System.out.println(recipes.get(i).recipe_title);
		    for(int p=0;p < recipes.get(i).recipe_materials.size();p++) {
		    	System.out.println(recipes.get(i).recipe_materials.get(p));　//recipesっていうリストからゲットしたrecipeクラスの中のrecipe_materialsをgetする
		    }
		}*/
	}
}
